...
struct geni_spi_desc {
	...
	int (*geni_spi_set_rate)(struct device *dev, unsigned long clk_freq);
	int (*geni_spi_switch_state)(struct device *dev, bool state);
};
...
static void spi_geni_rx_init_dma(struct geni_se *se, dma_addr_t iova, size_t len)
{
	u32 val;
...
static void spi_geni_handle_err(struct spi_master *spi, struct spi_message *msg)
{
	...
	case GENI_GPI_DMA:
		...
		break;
	default:
		dev_err(mas->dev, "Abort on Mode:%d not supported\n", mas->cur_xfer_mode);
	}
}
...
static int geni_spi_set_clock_and_bw(struct device *dev, unsigned long clk_hz)
{
					...
					struct spi_message *spi_msg)
{
	...
	dev_err(mas->dev, "Mode not supported %d\n", mas->cur_xfer_mode);
	return -EINVAL;
}
				...
				u16 mode, struct spi_master *spi)
{
	...
	ret = mas->dev_data->geni_spi_set_rate(mas->se.dev, xfer->speed_hz);
	if (ret)
		return ret;
	...
	if (mas->cur_xfer_mode == GENI_SE_DMA) {
		if (m_cmd & SPI_RX_ONLY)
			spi_geni_rx_init_dma(se, sg_dma_address(xfer->rx_sg.sgl),
				sg_dma_len(xfer->rx_sg.sgl));
		if (m_cmd & SPI_TX_ONLY)
			spi_geni_tx_init_dma(se, sg_dma_address(xfer->tx_sg.sgl),
...
int geni_spi_pm_domain_attach_list(struct device *dev,
...
void geni_spi_pm_domain_detach_list(struct dev_pm_domain_list *list)
{
	int i;
...
static int geni_spi_set_level(struct device *dev, unsigned long clk_freq)
{
	struct spi_controller *spi = dev_get_drvdata(dev);
	struct spi_geni_master *mas = spi_controller_get_devdata(spi);
	struct device *perf_dev = mas->pd_list->pd_devs[DOMAIN_IDX_PERF];
...
static int geni_spi_resource_off(struct spi_geni_master *mas)
{
	int ret;

	/* Drop the performance state vote */
	dev_pm_opp_set_rate(mas->dev, 0);
...
static int geni_spi_resource_on(struct spi_geni_master *mas)
{
	int ret;
...
static int geni_spi_resource_state(struct device *dev, bool power_on)
{
	struct spi_controller *spi = dev_get_drvdata(dev);
	struct spi_geni_master *mas = spi_controller_get_devdata(spi);
	int ret;
	...
	if (power_on)
		ret = geni_spi_resource_on(mas);
	else
		ret = geni_spi_resource_off(mas);

	if (ret)
		dev_err(mas->dev, "failed to switch resource power_on(power_on=%d) ret=%d\n",
			power_on, ret);
	return ret;
}

static int geni_spi_resource_init(struct spi_geni_master *mas)
{
	int ret;
...
static int spi_geni_probe(struct platform_device *pdev)
{
	...
	if (mas->dev_data->pd_data.num_pd_names) {
		ret = geni_spi_pm_domain_attach_list(&pdev->dev, &mas->dev_data->pd_data,
						&mas->pd_list);
		if (ret < 0)
			return ret;

		ret = pm_runtime_resume_and_get(mas->pd_list->pd_devs[DOMAIN_IDX_PERF]);
		if (ret)
			return ret;
	} else {
		ret = geni_spi_resource_init(mas);
		if (ret)
			return ret;
	}
...
spi_geni_probe_runtime_disable:
	...
	geni_spi_pm_domain_detach_list(mas->pd_list);
	return ret;
}
...
static int __maybe_unused spi_geni_runtime_suspend(struct device *dev)
{
	...
	return mas->dev_data->geni_spi_switch_state(dev, false);
}
...
static int __maybe_unused spi_geni_runtime_resume(struct device *dev)
{
	...
	return mas->dev_data->geni_spi_switch_state(dev, true);
}
...
static const struct geni_spi_desc geni_spi = {
	.geni_spi_set_rate = geni_spi_set_clock_and_bw,
	.geni_spi_switch_state = geni_spi_resource_state,
};
...
static const struct geni_spi_desc sa8255p_geni_spi = {
	...
	.geni_spi_switch_state = geni_spi_power_state,
};
