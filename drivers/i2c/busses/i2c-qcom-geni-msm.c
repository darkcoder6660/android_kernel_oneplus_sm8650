...
static int geni_i2c_resources_off(struct geni_i2c_dev *gi2c)
{
	int ret;
...
static int geni_i2c_resources_on(struct geni_i2c_dev *gi2c)
{
	int ret;
...
static int geni_i2c_resource_init(struct geni_i2c_dev *gi2c)
{
	struct device *dev = gi2c->se.dev;
	const struct geni_i2c_desc *desc = device_get_match_data(gi2c->se.dev);
	int ret;
...
static int geni_i2c_probe(struct platform_device *pdev)
{
	...
	if (!gi2c->dev_data->remotely_control) {
		ret = geni_i2c_resource_init(gi2c);
		if (ret)
			return ret;
	}
...
static int __maybe_unused geni_i2c_runtime_suspend(struct device *dev)
{
	...
	if (!gi2c->dev_data->remotely_control) {
		ret = geni_i2c_resources_off(gi2c);
		if (ret) {
...
static int __maybe_unused geni_i2c_runtime_resume(struct device *dev)
{
	...
	if (!gi2c->dev_data->remotely_control) {
		ret = geni_i2c_resources_on(gi2c);
		if (ret) {
			dev_err(dev, "failed to turn on resources, ret:%d\n", ret);
			return ret;
		}
	}
...
static const struct geni_i2c_desc geni_i2c = {
	.icc_ddr = "qup-memory",
	.remotely_control = false,
};

static const struct geni_i2c_desc sa8255p_control_geni_i2c = {
	.icc_ddr = "qup-memory",
	.remotely_control = true,
};

static const struct of_device_id geni_i2c_dt_match[] = {
	{ .compatible = "qcom,sa8255p-geni-i2c", .data = &sa8255p_control_geni_i2c },
	{}
};
