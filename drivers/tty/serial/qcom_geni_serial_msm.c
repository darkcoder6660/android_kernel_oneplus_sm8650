...
struct qcom_geni_device_data {
	...
	int (*geni_serial_switch_state)(struct uart_port *uport, bool state);
};
...
static unsigned int qcom_geni_serial_get_mctrl(struct uart_port *uport)
{
	...
	serial_trace_log(uport->dev, "%s: geni_ios: %d, mctrl:%d\n",
...
static void handle_rx_uart(struct uart_port *uport, u32 bytes, bool drop)
{
	...
	trace_serial_transmit_data_rx(uport->dev, port->rx_buf, bytes);
}
...
static void qcom_geni_serial_start_tx_dma(struct uart_port *uport)
{
	...
	if (pending) {
		serial_trace_log(uport->dev, "%s: TX transfer length: %d\n",
				 __func__, pending);
		trace_serial_transmit_data_tx(uport->dev, (char *)&xmit->buf[xmit->tail], pending);
	}
...
static irqreturn_t qcom_geni_serial_isr(int isr, void *dev)
{
	...
	serial_trace_log(uport->dev, "%s: s_irq_status: 0x%x, m_irq_status: 0x%x",
	...
	if (s_irq_status & S_RX_FIFO_WR_ERR_EN) {
		...
		if (!uart_console(uport))
			serial_trace_log(uport->dev, "%s: overrun: %d\n",
					 __func__,  uport->icount.overrun);
	}
	...
	if (s_irq_status & (S_GP_IRQ_0_EN | S_GP_IRQ_1_EN)) {
		if (s_irq_status & S_GP_IRQ_0_EN)
			uport->icount.parity++;
		if (!uart_console(uport))
			serial_trace_log(uport->dev, "%s: parity: %d\n",
					 __func__, uport->icount.parity);
		...
		port->brk = true;
		if (!uart_console(uport))
			serial_trace_log(uport->dev, "%s: break: %d\n",
					 __func__, uport->icount.brk);
	}
...
int geni_serial_pm_opp_set_level(struct device *dev, unsigned int level)
{
					 ...
					 const struct ktermios *old)
{
	...
	if (port->dev_data->geni_serial_set_rate) {
		ret = port->dev_data->geni_serial_set_rate(uport, port->baud);
		if (ret) {
			dev_err(port->se.dev,
...
static struct uart_driver qcom_geni_console_driver = {
	.owner = THIS_MODULE,
	.driver_name = "qcom_geni_console_msm",
	.dev_name = "ttyMSM",
...
static struct uart_driver qcom_geni_uart_driver = {
	.owner = THIS_MODULE,
	.driver_name = "qcom_geni_uart_msm",
	.dev_name = "ttyHS",
	.nr =  GENI_UART_PORTS,
};

int geni_serial_pm_domain_attach_list(struct device *dev,
...
void geni_serial_pm_domain_detach_list(struct dev_pm_domain_list *list)
{
	int i;
...
static int geni_serial_resource_off(struct uart_port *uport)
{
	struct qcom_geni_serial_port *port = to_dev_port(uport);
	int ret;
...
static int geni_serial_resource_on(struct uart_port *uport)
{
	struct qcom_geni_serial_port *port = to_dev_port(uport);
	int ret;
...
static int geni_serial_resource_state(struct uart_port *uport, bool power_on)
{
	...
	if (power_on)
		ret = geni_serial_resource_on(uport);
	else
		ret = geni_serial_resource_off(uport);

	if (ret)
		dev_err(uport->dev, "failed to switch resource power_on(power_on=%d) ret=%d\n",
			power_on, ret);
	return ret;
}
		...
		unsigned int new_state, unsigned int old_state)
{
	...
	if (port->dev_data->geni_serial_switch_state)
		port->dev_data->geni_serial_switch_state(uport, power_on);
}
...
static int qcom_geni_serial_probe(struct platform_device *pdev)
{
	...
	if (port->dev_data->pd_data.num_pd_names) {
		ret = geni_serial_pm_domain_attach_list(&pdev->dev,
						&port->dev_data->pd_data, &port->pd_list);
		if (ret < 0)
			return ret;
		ret = pm_runtime_resume_and_get(port->pd_list->pd_devs[DOMAIN_IDX_PERF]);
		if (ret)
			goto error;
	} else {
...
error:
	geni_serial_pm_domain_detach_list(port->pd_list);
	return ret;
}
...
static const struct qcom_geni_device_data qcom_geni_console_data = {
	...
	.geni_serial_switch_state = geni_serial_resource_state,
};
...
static const struct qcom_geni_device_data sa8255p_qcom_geni_console_data = {
	...
	.geni_serial_switch_state = geni_serial_power_state,
};
